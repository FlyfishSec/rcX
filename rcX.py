...
def Generator(host="127.0.0.1", port="44444", port2="", shell_type="bash", shell_path="", platform="[]", password="",
              ...
              staging_url=None, localtunnel=None, authtoken=""):
    if host == "":
        host = "127.0.0.1"
    if port == "":
        port = "44444"
...
def create_app(host="127.0.0.1", port=80, debug=False, template='', authtoken=''):
    ...
    def gen():
        ...
        payload = Generator(host=_host, port=_port, shell_type=shell_type, direction=direction, protocol=protocol,
                            ...
                            localtunnel=localtunnel, authtoken=authtoken)
        title = None
        ...
        if any(x in request.headers["User-Agent"].lower() for x in ua):
            ...
            if template != 1:
                if os.path.exists(template):
                    if os.path.isdir(template):
                        html = send_from_directory(os.path.abspath(template), 'index.html')
                    else:
                        html = send_file(os.path.abspath(template))
